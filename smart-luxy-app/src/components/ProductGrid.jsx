...
export default function ProductGrid({
  ...
  // Charger les moyennes des avis pour tous les produits
      ...
      <div className="cats">
        {categories.map(cat => (
          <button key={cat} className={`cat-btn ${activeCat === cat ? 'active' : ''}`}
            onClick={() => onCatChange(cat)}>{cat}</button>
        ))}
      </div>
...
function ProductCard({ product: p, reviewData, onOpen, onAddToCart, onBuyNow }) {
  ...
  return (
    <div className="pcard" style={{ opacity: outOfStock ? 0.65 : 1, background: cardBg }}>
      ...
      {/* ── Image ── */}
      <div className="pcard-img" onClick={() => onOpen(p)} style={{ position: 'relative' }}>
        {mainImg
        ...
        {disc > 0 && (
          <div className="pcard-badge" style={{ left:'auto', right:10, background:'#ef4444' }}>-{disc}%</div>
        )}
        ...
        {outOfStock && (
          <div style={{
            ...
            fontSize:11, fontWeight:800, padding:'5px 0', textAlign:'center', letterSpacing:'.06em',
          }}>ÉPUISÉ</div>
        )}
        {lowStock && !outOfStock && (
          <div style={{
        ...
        {/* Miniatures images en bas de l'image principale */}
        {imgs.length > 1 && (
          <div style={{
            position:'absolute', bottom: outOfStock ? 28 : (lowStock ? 34 : 6),
            ...
            {imgs.length > 4 && (
              <div style={{ width:18, height:18, borderRadius:3, background:'rgba(0,0,0,.7)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:8, color:'white', fontWeight:800 }}>
                +{imgs.length-4}
              </div>
            )}
          </div>
      ...
      <div className="pcard-body">
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:6, minHeight:16 }}>
          {reviewData ? (
            ...
            <div style={{ display:'flex', gap:1 }}>
              {[1,2,3,4,5].map(i => <span key={i} style={{ fontSize:11, color:'rgba(255,255,255,.1)' }}>★</span>)}
            </div>
          )}
          {p.ventes > 0 && (
